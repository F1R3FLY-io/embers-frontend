...
export class AiAgent {
    ...
    public async createAgent(agentReq: CreateAgentReq) {
        ...
        const sendContract: DeployContractCallback = async ({ contract, sig, sigAlgorithm }) => {
    ...
    public async deployAgent(agentId: string, version: string) {
        ...
        const contract: GetContractCallback = async () => {
            ...
            const prepareResponse = await this.client.apiAiAgentsAddressIdVersionDeployPreparePost({
        ...
        // Send the signed contract
        const sendConract: DeployContractCallback = async ({ contract, sig, sigAlgorithm }) => {
            const signedContract: SignedContract = {
                contract: Array.from(contract),
                deployer: Array.from(this.privateKey.getPublicKey().value),
                sig: Array.from(sig),
