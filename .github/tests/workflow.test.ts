...
describe('CI/CD Workflow Consistency', () => {
  ...
  describe('Local Development Workflow', () => {
    ...
    test('frontend should use workspace:* dependency for client SDK', () => {
      ...
      expect(appsPackageJson.dependencies['@f1r3fly-io/embers-client-sdk'])
        .toBe('workspace:*');
    });
    ...
    test('pnpm dev should build SDK first, then start frontend', () => {
      ...
      expect(rootPackageJson.scripts.dev)
        .toMatch(/embers-client-sdk build.*embers-frontend dev/);
    });
  ...
  describe('Pull Request Workflow Validation', () => {
    ...
    test('workspace configuration enables proper package resolution', () => {
      ...
      const nodeModulesPath = path.join(appsPackageDir, 'node_modules', '@f1r3fly-io', 'embers-client-sdk');
  ...
  describe('Workflow Consistency Validation', () => {
    test('all workflows should use same pnpm and node versions', () => {
      ...
      const clientPnpmVersion = clientWorkflow.match(/pnpm\/action-setup@v\d[\s\S]*?version:\s*(\d+)/)?.[1];
      const embersPnpmVersion = embersWorkflow.match(/pnpm\/action-setup@v\d[\s\S]*?version:\s*(\d+)/)?.[1];
    ...
    test('build commands should be consistent across workflows', () => {
      ...
      expect(clientWorkflow).toMatch(new RegExp(clientBuildCmd.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
      expect(embersWorkflow).toMatch(new RegExp(clientBuildCmd.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
    });
  ...
  describe('Registry and Authentication Configuration', () => {
    ...
    test('client SDK should only have standard npm dependencies', () => {
      ...
      const problematicDeps = prodDeps.filter(dep => 
        dep.startsWith('@f1r3fly-io/') && dep !== '@f1r3fly-io/embers-client-sdk'
      );
    ...
    test('frontend app should only reference external @f1r3fly-io packages that exist on standard npm', () => {
      ...
      const unexpectedDeps = externalF1r3flyDeps
        .filter(([name]) => !expectedExternalDeps.includes(name));
